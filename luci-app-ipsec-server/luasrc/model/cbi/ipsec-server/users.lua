...
local type = m:get("ipsec", "type")

if type == "IKEv2" then
	s = m:section(TypedSection, "ikev2_users", "IKEv2/IPSec PSK " .. translate("Users Manager"))
	s.description = translate("Use a client that supports IKEv2/IPSec PSK (iOS or Android) to connect to this server.")
	s.addremove = true
	s.anonymous = true
	...
	o = s:option(Value, "identifier", translate("Identifier"))
	o.placeholder = translate("Identifier")
	o.rmempty = false

	o = s:option(Value, "secret", translate("Preshared Key"))
	o.placeholder = translate("Preshared Key")
	o.rmempty = false
end
if type == "IKEv1" then
...
if type == "L2TP" and sys.call("command -v xl2tpd > /dev/null") == 0 then
